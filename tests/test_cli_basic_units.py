...
def _test_cli_op(operation : str, model : str, config : str = None):
    if config:
        result = subprocess.run(['flamapy', operation, model, config], capture_output=True, text=True)
    else:
        result = subprocess.run(['flamapy', operation, model], capture_output=True, text=True)
    return result

@pytest.mark.parametrize("operation, model, config, res_type, expected_output", [
    ('atomic_sets', VALID_MODEL, None, "list", "[['PHP', 'Search', 'Catalog', 'Security', 'Payment', 'eCommerce', 'Shopping', 'Web', 'Storage', 'v74', 'Cart', 'Server'], ['LOW'], ['ENOUGH'], ['BASIC'], ['ADVANCED'], ['PayPal'], ['CreditCard'], ['Mobile'], ['HIGH'], ['STANDARD'], ['Backup'], ['Marketing'], ['SEO'], ['Socials'], ['Twitter'], ['Facebook'], ['YouTube']]"),
    ('average_branching_factor', VALID_MODEL, None, "float", "2.45"),
    ('core_features', VALID_MODEL, None, "list", "['eCommerce', 'Server', 'Web', 'Catalog', 'Search', 'Shopping', 'Security', 'Cart', 'Payment', 'PHP', 'Storage', 'v74']"),
    ...
    ('leaf_features', VALID_MODEL, None, "list", "['v74', 'LOW', 'ENOUGH', 'Catalog', 'BASIC', 'ADVANCED', 'Cart', 'PayPal', 'CreditCard', 'Mobile', 'HIGH', 'STANDARD', 'Backup', 'SEO', 'Twitter', 'Facebook', 'YouTube']"),
