...
from flamapy.metamodels.pysat_metamodel.operations.pysat_configurations_number import PySATConfigurationsNumber
from flamapy.metamodels.pysat_metamodel.operations.pysat_configurations import PySATConfigurations
...
from flamapy.metamodels.pysat_metamodel.operations.pysat_satisfiable_configuration import PySATSatisfiableConfiguration
...
def test_false_optional_operation(setup_model, expected_output):
    ...
    result_names = [str(feature) for feature in result]

    assert result_names == expected_output, f"The result {result_names} does not match with the expected output {expected_output}"
...
def test_number_of_configurations_operation(setup_model, expected_output):
    model = setup_model
    operation = PySATConfigurationsNumber()
    operation.execute(model)
